...
class FavouriteListCreateApiView(ListCreateAPIView):
    ...
    def get_serializer_context(self):
        ...
        u_images = {img.user_id: img for img in Image.objects.filter(is_profile=True).select_related('user').filter(user__in=[obj.user for obj in self.get_queryset()])}
        o_images = {img.user_id: img for img in Image.objects.filter(is_profile=True).select_related('user').filter(user__in=[obj.other for obj in self.get_queryset()])}
        context.update({'u_images': u_images, 'o_images': o_images})
        return context
    ...
    def list(self, request, *args, **kwargs):
        ...
        if like is not None and like == 'ifav':
        ...
        if like is not None and like == 'theyfav':
...
class UserViewedListCreateApiView(ListCreateAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if like is not None and like == 'iview':
            queryset_i_like = queryset.filter(
                user=self.request.user).order_by('-id')
            serializer = UserViewedSerializer(
                data_slice(queryset_i_like, count), many=True)
        ...
        if like is not None and like == 'theyview':
            queryset_they_like = queryset.filter(
                other=self.request.user).order_by('-id')
            serializer = UserViewedSerializer(
                data_slice(queryset_they_like, count), many=True)
